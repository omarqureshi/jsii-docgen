...
test.each([
  ...
  // The minimum-rosetta floor only means something against a *released*
  // rosetta. Local fork builds (the file:../jsii-rosetta pin) report version
  // 0.0.0, which can never satisfy a floor like >=5.9.5 — for those builds
  // the floor is waived and the test always runs, which is strictly stricter.
  if (ROSETTA_VERSION !== '0.0.0' && !semver.satisfies(ROSETTA_VERSION, minimumRosettaRange)) {
